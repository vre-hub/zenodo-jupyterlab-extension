...
@pytest.mark.asyncio
@patch('zenodo_jupyterlab.server.search.search_records', new_callable=AsyncMock)
async def test_searchRecords_success(mock_search_records):
    mock_search_records.return_value = [
        AsyncMock(id='1', title='Record One', metadata={'publication_date': '2022-01-01', 'resource_type': {'title': 'Dataset'}}),
        AsyncMock(id='2', title='Record Two', metadata={'publication_date': '2023-01-01', 'resource_type': {'title': 'Article'}})
    ]
...
@pytest.mark.asyncio
@patch('zenodo_jupyterlab.server.search.search_communities', new_callable=AsyncMock)
...
@pytest.mark.asyncio
@patch('zenodo_jupyterlab.server.search.get_record', new_callable=AsyncMock)
